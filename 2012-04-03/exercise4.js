...
var base = COLOR([0.9,0.9,0.9])(STRUCT([base1,base2,base3,base4,base5,base6,base7,base8,base9,base10,base11,bL1,bR1,bR2,bB1,bB2,stairs]));
...
COLOR([0.6,0.8,0.6]) (extRight);

//parete esterna bianca
var extWhiteWall = SIMPLEX_GRID ([[-6.5,20],[-15,thickness],[-1,h]]);
COLOR([0.9,0.9,0.9]) (extWhiteWall);

//Struct comprendente le pareti esterne
var extwalls = STRUCT([extLeft,extRight,extWhiteWall]);

//pareti interne
var intWall1 = COLOR([0,0.4,0])(SIMPLEX_GRID ([[-25,9],[-7.5+thickness,thickness],[-1,h]]));
var intWall2 = COLOR([0.5,0.27,0.07])(SIMPLEX_GRID ([[-37.5,5],[-11.4,thickness],[-1,h]]));
...
COLOR([0.4,0.4,0.4])(setFrames);


//EXERCISE 4
//Reproduce a 3D Model of one piece of furniture, at your choice, and istance it one or more times
//inside the Pavillion
//E' stato scelto di realizzare uno scaffale posizionato nella saletta di fronte la vasca più ampia
//si consiglia, come indicato sotto, di rimuovere il tetto dallo struct model per visualizzare in maniera
//più immediata il posizionamento degli oggetti
var laterali = SIMPLEX_GRID([ REPLICA(1)([0.1,-1,0.1]), [0.4], [2.1] ]);
var ripiani = SIMPLEX_GRID([ [-0.1,1,-0.1], [0.4], REPLICA(6)([0.1,-0.3]) ]);
var fondo = SIMPLEX_GRID([ [1.2], [-0.4,0.1], [2.1] ]);
var scaffale = STRUCT ([laterali,ripiani,fondo])
COLOR([0.54,0.27,0.14])(scaffale);

var tlibrerie = T([0])([1.4]);
var libreria = STRUCT([scaffale, tlibrerie, scaffale,tlibrerie, scaffale]);
var libreria_placed = T([0,1,2])([1.5,21.2,1])(libreria);


//rimuovere lo struct "roofs" (l'ultimo) dal model per poter visualizzare il modello senza tetto e verificare
//il posizionamento delle pareti interne e dei pannelli
var model = STRUCT([baseVasche,extwalls,intWalls,setFrames,libreria_placed, roofs]);
